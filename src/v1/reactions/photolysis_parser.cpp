        ...
        types::Reactions& reactions)
    {
      ...
      if (validate.empty())
      {
        ...
        if (reactants.second.size() > 1)
        {
          ...
          errors.push_back({ ErrorCode::TooManyReactionComponents, line + ":" + column + ": Too many reactants" });
        }

        photolysis.gas_phase = gas_phase;
        photolysis.products = products.second;
        photolysis.reactants = reactants.second.empty() ? types::ReactionComponent{} : reactants.second[0];
