    ...
    if (!duplicates.empty())
    {
      for (const auto& duplicate : duplicates)
      {
        ...
        for (size_t i = 0; i < total; ++i)
        {
          ...
          errors.push_back({ ErrorCode::DuplicateSpeciesDetected, oss.str() });
        }
      }
    }
      ...
      if (validate.empty())
      {
        ...
        for (const auto& spec : object[validation::species])
        {
          ...
          else if (spec.IsMap())
          {
            ...
            {
              errors.push_back(
                  { ErrorCode::RequiredKeyNotFound, FormatYamlError(spec, "Species object missing required 'name' field") });
              continue;
            }
          ...
          for (size_t j = i + 1; j < species.size(); ++j)
          {
            if (species[i].name == species[j].name)
            {
              ...
              oss << line << ":" << column << " error: Duplicate species '" << species[i].name << "' found in '" << name
                  << "' phase";
              errors.push_back({ ErrorCode::DuplicateSpeciesInPhaseDetected, oss.str() });
            }
          }
        }
        ...
        if (!unknown_species.empty())
        {
          ...
          errors.push_back({ ErrorCode::PhaseRequiresUnknownSpecies, oss.str() });
        }
    ...
    if (!duplicates.empty())
    {
      for (const auto& duplicate : duplicates)
      {
        ...
        for (size_t i = 0; i < total; ++i)
        {
          ...
          errors.push_back({ ErrorCode::DuplicatePhasesDetected, oss.str() });
        }
      }
    }
      ...
      const std::vector<types::Phase>& existing_phases)
  {
    ...
    for (const auto& object : objects)
    {
      ...
      else
      {
        ...
        errors.push_back({ ErrorCode::UnknownType, "Unknown type: " + type + " at line " + line + " column " + column });
      }
    }
