...
TEST(TernaryChemicalActivationConfig, ParseConfig)
{
  ...
  for (auto& extension : extensions)
  {
    ...
    // first reaction
    {
      ...
      EXPECT_EQ(process_vector[0].k0_A, 1.0 * std::pow(conversions::MOLES_M3_TO_MOLECULES_CM3, 3));
      EXPECT_EQ(process_vector[0].k0_B, 0.0);
      EXPECT_EQ(process_vector[0].k0_C, 0.0);
      EXPECT_EQ(process_vector[0].kinf_A, 1.0 * std::pow(conversions::MOLES_M3_TO_MOLECULES_CM3, 2));
    ...
    // second reaction
    {
      ...
      EXPECT_EQ(process_vector[1].k0_A, 32.1 * std::pow(conversions::MOLES_M3_TO_MOLECULES_CM3, 2));
      EXPECT_EQ(process_vector[1].k0_B, -2.3);
      EXPECT_EQ(process_vector[1].k0_C, 102.3);
      EXPECT_EQ(process_vector[1].kinf_A, 63.4 * std::pow(conversions::MOLES_M3_TO_MOLECULES_CM3, 1));
