...
TEST(SurfaceConfig, ParseConfig)
{
  ...
  for (auto& extension : extensions)
  {
    ...
    // first reaction
    {
      EXPECT_EQ(process_vector[0].gas_phase_species.name, "foo");
      EXPECT_EQ(process_vector[0].gas_phase_products.size(), 2);
      EXPECT_EQ(process_vector[0].gas_phase_products[0].name, "bar");
      ...
      auto it = std::find_if(
          ...
          mechanism.species.end(),
          [](const types::Species& species) { return species.name == "foo"; });
    ...
    // second reaction
    {
      EXPECT_EQ(process_vector[1].gas_phase_species.name, "bar");
      EXPECT_EQ(process_vector[1].gas_phase_products.size(), 2);
      EXPECT_EQ(process_vector[1].gas_phase_products[0].name, "bar");
      ...
      auto it = std::find_if(
          mechanism.species.begin(),
          mechanism.species.end(),
          [](const types::Species& species) { return species.name == "bar"; });
      ASSERT_NE(it, mechanism.species.end());
